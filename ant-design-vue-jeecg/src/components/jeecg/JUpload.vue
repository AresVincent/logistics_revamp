    ...
    methods:{
      ...
      handlePathChange(){
        ...
        for(var a=0;a<uploadFiles.length;a++){
          // update-begin-author:lvdandan date:20200603 for:【TESTA-514】【開源issue】多個文件同時上傳時，控制台報錯
          if(uploadFiles[a].status === 'done' ) {
            arr.push(uploadFiles[a].response.message)
          }else{
            return;
          }
          // update-end-author:lvdandan date:20200603 for:【TESTA-514】【開源issue】多個文件同時上傳時，控制台報錯
        }
        if(arr.length>0){
          path = arr.join(",")
        }
      ...
      handleChange(info) {
        ...
        if(info.file.status==='done' || info.file.status === 'removed'){
          //returnUrl為true時僅返迴文件路徑
          if(this.returnUrl){
            this.handlePathChange()
          }else{
            //returnUrl為false時返迴文件名稱、文件路徑及文件大小
            this.newFileList = [];
