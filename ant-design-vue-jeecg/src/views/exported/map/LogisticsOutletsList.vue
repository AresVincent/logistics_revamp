        ...
        <a-row :gutter="24">
          <a-col :xl="6" :lg="7" :md="8" :sm="24">
            <a-form-item label="自提點/自提櫃">
              <j-dict-select-tag placeholder="請選擇自提點/自提櫃" v-model="queryParam.type" dictCode="self_station"/>
            </a-form-item>
          </a-col>
          <a-col :xl="6" :lg="7" :md="8" :sm="24">
            <a-form-item label="地區ID">
              <j-dict-select-tag placeholder="請選擇地區ID" v-model="queryParam.areaId" dictCode="common_area,area,id"/>
            </a-form-item>
          </a-col>
          <a-col :xl="6" :lg="7" :md="8" :sm="24">
  ...
  export default {
    ...
    data () {
      return {
          ...
          },
          {
            ...
            dataIndex: 'companyId',
             customRender:function(text,record,index){
              console.log(text)
              return filterDictText(companyOption,text)
            }
          },
          {
            title:'地區ID',
        ...
        url: {
          list: "/logistics/outlet/list",
          delete: "/logistics/outlet/delete",
          deleteBatch: "/logistics/outlet/deleteBatch",
          exportXlsUrl: "/logistics/outlet/exportXls",
          importExcelUrl: "/logistics/outlet/importExcel",
    ...
    watch:{
      dataSource:function(val){
        ...
        this.markers=markerArray.slice(0);
        console.log('標點',val)
      }
    },
    methods: {
      initDictConfig(){
        initDictOptions('express_company').then((res) => {
          if (res.success) {
            companyOption = res.result;
      ...
      getSuperFieldList(){
        let fieldList=[];
        fieldList.push({type:'string',value:'type',text:'自提點/自提櫃',dictCode:'self_station'})
        fieldList.push({type:'int',value:'companyId',text:'所屬快遞公司',dictCode:'express_company'})
        fieldList.push({type:'int',value:'areaId',text:'地區ID',dictCode:'common_area,area,id'})
